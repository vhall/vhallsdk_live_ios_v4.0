...
- (void)setupLayoutWithDict:(NSDictionary *)dict andWidth:(CGFloat)width{
    ...
    if (self.configure.cellBackgroundImage) {
        ...
        imageView.image = BundleUIImage(self.configure.cellBackgroundImage);
    ...
    if (self.type == SZQuestionOpenQuestion) {
        ...
        for (int i = 0; i < optionArray.count; i++) {
            ...
            if (currentLabel == nil) {
                ...
                [btn setImage:BundleUIImage(isSingleChoice ? self.configure.unCheckedImage : self.configure.multipleUncheckedImage) forState:UIControlStateNormal];
                [btn setImage:BundleUIImage(isSingleChoice ? self.configure.checkedImage : self.configure.multipleCheckedImage) forState:UIControlStateSelected];
