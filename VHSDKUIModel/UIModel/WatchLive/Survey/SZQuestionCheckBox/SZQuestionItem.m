...
- (instancetype)initWithTitleArray:(NSArray *)titleArray andOptionArray:(NSArray *)optionArray andResultArray:(NSArray *)resultArray andQuestonType:(SZQuestionItemType)type {
    ...
    NSArray *typeArray = [self creatTypeArray:titleArray type:type];
    return [self initWithTitleArray:titleArray andOptionArray:optionArray andResultArray:resultArray andQuestonTypes:typeArray isMustSelectArray:nil];
}

- (instancetype)initWithTitleArray:(NSArray *)titleArray andOptionArray:(NSArray *)optionArray  andQuestonTypes:(NSArray *)typeArray {
    
     return [self initWithTitleArray:titleArray andOptionArray:optionArray andResultArray:nil andQuestonTypes:typeArray isMustSelectArray:nil];
}
    ...
    if (self = [super init]) {
        ...
        self.questionArray = [self creatItemWithTitleArray:titleArray
                                         ...
                                         andandResultArray:resultArray
                                           andQuestonTypes:typeArray isMustSelectArray:ismustSelectarray];
    }
    return self;
}
...
- (NSArray *)creatItemWithTitleArray:(NSArray *)titleArray andOptionArray:(NSArray *)optionArray andandResultArray:(NSArray *)resultArray andQuestonTypes:(NSArray *)typeArray isMustSelectArray:(NSArray*)isMustSelectArray {
    ...
    for (int i = 0; i < titleArray.count; i++) {
        ...
        NSString  *isMustSelect = isMustSelectArray[i];
        if (type == SZQuestionOpenQuestion) {
            
            NSString *answerString = resultArray == nil ? @"" : resultArray[i];
            NSDictionary *dict = [NSDictionary dictionaryWithObjectsAndKeys:titleArray[i], @"title", answerString, @"marked", typeArray[i], @"type", isMustSelect,@"isMustSelect",nil];;
            [arrayM addObject:dict];
        }
        else {
            
            NSArray *answerArray = resultArray == nil ? [self createSelectArray:optionArray[i]] : resultArray[i];
            NSDictionary *dict = [NSDictionary dictionaryWithObjectsAndKeys:titleArray[i], @"title", optionArray[i], @"option", answerArray, @"marked", typeArray[i], @"type", isMustSelect,@"isMustSelect",nil];
            [arrayM addObject:dict];
        }
    }
    return arrayM.copy;
