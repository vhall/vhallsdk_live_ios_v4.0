...
- (void)keyboardShow:(NSNotification *)notification
{
    ...
    [UIView animateWithDuration:duration animations:^{
        self.frame = CGRectMake(0, VHScreenHeight - keyboradHeight - InputViewHeight, VHScreenWidth, InputViewHeight);
...
- (void)keyboardToolViewBackViewClick {
    ...
    if(self.emojiBtn.selected) { //当前处于输入emoji表情
        [UIView animateWithDuration:0.3 animations:^{
            self.frame = CGRectMake(0, VHScreenHeight, VHScreenWidth, InputViewHeight);
            self.faceView.frame = CGRectMake(0, VHScreenHeight, self.frame.size.width, FaceViewHeight);
        } completion:^(BOOL finished) {
            [self.keyboardBackView removeFromSuperview];
            self.hidden = YES;
            self.faceView.hidden = YES;
            self.faceView.frame = CGRectMake(0, VHScreenHeight - FaceViewHeight, self.frame.size.width, FaceViewHeight);
        }];
    }
}
...
- (void)showFaceView {
    ...
    self.frame = CGRectMake(0, VHScreenHeight - FaceViewHeight - InputViewHeight, VHScreenWidth, InputViewHeight);
}
...
- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text
{
    ...
    if([UIModelTools hasEmoji:text]) { //屏蔽第三方键盘表情
        return NO;
    }
...
- (VHFaceView *)faceView
{
    if (!_faceView)
    {
        _faceView = [[VHFaceView alloc] initWithFrame:CGRectMake(0, VHScreenHeight - FaceViewHeight, self.frame.size.width, FaceViewHeight)];
...
- (UIButton *)sendBtn
{
    if (!_sendBtn)
    {
        ...
        _sendBtn.backgroundColor = MakeColorRGB(0xFB3A32);
...
- (VHTextView *)textView
{
    if (!_textView)
    {
        ...
        _textView.placeholderTextColor = MakeColorRGB(0x999999);
