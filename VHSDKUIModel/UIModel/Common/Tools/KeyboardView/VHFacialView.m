...
@interface VHFacialView ()
{
    float FaceSizeWidth;
    float FaceSizeHeight;
    float EdgeDistance; //两边边缘间隔
    float EdgeInterVal; //上下边缘间隔
}
@end

@implementation VHFacialView
...
//    NSLog(@"%f,%f",verticalInterval,CGRectGetHeight(self.bounds));
    ...
    for (int i = 0; i<Lines; i++)
    {
        for (int x = 0;x<NumPerLine;x++)
        {
            ...
            if (i*7+x+1 ==21) {
                [expressionButton setImage:BundleUIImage(@"faceDelete")
...
- (void)faceClick:(UIButton *)button
{
    if (button.tag == 10000 && _delegate) {
        ...
        NSString *expressstring = [NSString stringWithFormat:@"Expression_%ld",button.tag];
